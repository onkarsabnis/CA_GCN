...
class MatchNetwork(nn.Module):
    ...
    def forward(self, data_batch):
        ...
        for i in range(len(adj_list)):
            ...
            if self.GCNModel:
                print("seq1_len ", seq1_len)
                print("seq2_len ",seq2_len)
                print("seq1_len_curr ",seq1_len_curr)
                ...
                # print("seq1_len ", seq1_len)
                # print("seq2_len ",seq2_len)
                # print("seq2_len_curr ",seq2_len_curr)
                # print("seq1_len + seq2_len - seq2_len_curr ", seq1_len + seq2_len - seq2_len_curr)
                # seq2_len_curr
                adj2_curr = torch.cat((adj2_curr, torch.zeros(adj2_curr.size(0), seq2_len - seq2_len_curr, seq2_len_curr+2).to(self.device)), 1)
                adj2_curr = torch.cat((adj2_curr, torch.zeros(adj2_curr.size(0), seq2_len+2, seq1_len + seq2_len - seq2_len_curr).to(self.device)), 2)

                # print(adj1_curr.size())
                # print(adj2_curr.size())
                adj2_curr_copy = torch.flatten(adj2_curr)
                # print(adj2_curr_copy.size())
                adj2_curr = adj2_curr_copy[:6272]
                # print(adj2_curr.view(4,28,56).size())
                adj_curr = torch.cat((adj1_curr, adj2_curr.view(4,28,56)), 1)
...
def main():
    ...
    hyps["train_len"] = 3000
    hyps["warmup_steps"] = int((3000 / hyps["batch_size"]) / 10)
    hyps["dropout_inter"] = 0
    hyps["input_data_list"] = ['../dependency_scientsbank_train_matched.tsv']
    hyps["input_label_list"] = ['../scientsbank_train_labels.txt']
    ...
    hyps["eval_model_folder"] = "results/bert_co_attn/seed69"  # where to load the evaluated model
    ...
    t_total = int(3000 / hyps["batch_size"]) * hyps["epochs"]
